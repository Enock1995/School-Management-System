...
import {
  HeartPulse, Syringe, Pill as PillIcon, ClipboardPlus, Search, Plus, X,
  AlertTriangle, CheckCircle2, ShieldCheck, GraduationCap, Heart, Phone,
  Droplet, Info, Sparkles, Clock
} from "lucide-react";
...
function statusTone(s) {
  if (["Resolved", "Up to Date"].includes(s)) return "green";
  if (["Monitoring", "Due Soon"].includes(s)) return "amber";
  if (["Overdue", "High"].includes(s)) return "red";
  return "slate";
}
...
export default function App() {
  ...
  const roles = [
    { key: "admin", label: "Admin / Nurse", icon: ShieldCheck },
    { key: "teacher", label: "Teacher", icon: HeartPulse },
    { key: "student", label: "Student", icon: GraduationCap },
    { key: "parent", label: "Parent", icon: Heart },
  ];
  return (
    <div style={{ minHeight: "100vh", background: C.bgGrad, ...bodyFont, padding: 26 }}>
      <style>{`${FONT_IMPORT} * { box-sizing: border-box; } input::placeholder, textarea::placeholder { color: ${C.textFaint}; } select { cursor: pointer; }`}</style>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ ...displayFont, fontSize: 20, fontWeight: 700, color: C.text, margin: 0 }}>Health Management</h1>
            <p style={{ fontSize: 12.5, color: C.textMuted, margin: "2px 0 0" }}>Clinic visits, medical records, medication & vaccinations</p>
          </div>
        ...
        {role === "admin" && <HealthAdminView />}
        {role === "teacher" && <HealthTeacherView />}
        {(role === "student" || role === "parent") && <HealthPersonalView role={role} />}
      </div>
    </div>
  );
}